...
def persp2equir(frame_path, input_folder):
    ...
    # Load perspective image 1 if it exists, otherwise create an empty image
    if os.path.exists(input_img1):
        theta1, phi1 = load_pnginfo(input_img1)
        persp1 = P2E.Perspective(input_img1, 90, theta1, phi1)
        img1, _ = persp1.GetEquirec(orig_height, orig_width)
    else:
        img1 = np.zeros((orig_height, orig_width, 3), dtype=np.uint8)

    # Load perspective image 2 if it exists, otherwise create an empty image
    if os.path.exists(input_img2):
        theta2, phi2 = load_pnginfo(input_img2)
        persp2 = P2E.Perspective(input_img2, 90, theta2, phi2)
        img2, _ = persp2.GetEquirec(orig_height, orig_width)
    else:
    ...
    # Apply feathering to the masks
    feathered_mask1 = apply_feathering(mask1)
    feathered_mask2 = apply_feathering(mask2)
